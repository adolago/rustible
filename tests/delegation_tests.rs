...
async fn test_delegate_to_basic() {
    ...
    let task = Task::new("Debug on localhost", "debug")
        .arg("msg", "Hello from delegated task");
    ...
    let result = delegated_task.execute(&ctx, &runtime, &handlers, &notified, &parallelization).await;
...
async fn test_delegate_facts_false() {
    ...
    task.args.insert("test_var".to_string(), serde_json::json!("test_value"));
...
async fn test_delegate_facts_true() {
    ...
    task.args.insert("delegate_var".to_string(), serde_json::json!("delegate_value"));
    task.delegate_to = Some("localhost".to_string());
...
async fn test_delegate_facts_default_false() {
    ...
    task.args.insert("default_var".to_string(), serde_json::json!("default_value"));
    task.delegate_to = Some("localhost".to_string());
...
async fn test_delegate_with_register() {
    ...
    task.args.insert("msg".to_string(), serde_json::json!("Delegated message"));
    task.delegate_to = Some("localhost".to_string());
...
async fn test_no_delegation() {
    ...
    task.args.insert("normal_var".to_string(), serde_json::json!("normal_value"));
