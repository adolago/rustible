...
impl BenchResult {
    fn summary(&self) -> String {
        ...
        let avg_connect: Duration = self.connect_times.iter().sum::<Duration>() / self.connect_times.len() as u32;
        let avg_command: Duration = self.command_times.iter().sum::<Duration>() / self.command_times.len() as u32;
...
async fn bench_ssh2(iterations: usize) -> BenchResult {
    ...
    if !key_path.exists() {
        return BenchResult { failures: iterations, ..Default::default() };
    }
...
async fn compare_ssh_performance() {
    ...
    if !ssh2_result.connect_times.is_empty() && !russh_result.connect_times.is_empty() {
        let ssh2_avg_connect: Duration = ssh2_result.connect_times.iter().sum::<Duration>() / ssh2_result.connect_times.len() as u32;
        let russh_avg_connect: Duration = russh_result.connect_times.iter().sum::<Duration>() / russh_result.connect_times.len() as u32;

        let ssh2_avg_cmd: Duration = ssh2_result.command_times.iter().sum::<Duration>() / ssh2_result.command_times.len() as u32;
        let russh_avg_cmd: Duration = russh_result.command_times.iter().sum::<Duration>() / russh_result.command_times.len() as u32;

        let connect_ratio = ssh2_avg_connect.as_nanos() as f64 / russh_avg_connect.as_nanos() as f64;
        let cmd_ratio = ssh2_avg_cmd.as_nanos() as f64 / russh_avg_cmd.as_nanos() as f64;

        println!("\n=== Comparison ===");
        println!("Connect time: russh is {:.2}x {} than ssh2",
            if connect_ratio > 1.0 { connect_ratio } else { 1.0 / connect_ratio },
            if connect_ratio > 1.0 { "faster" } else { "slower" }
        );
        println!("Command time: russh is {:.2}x {} than ssh2",
            if cmd_ratio > 1.0 { cmd_ratio } else { 1.0 / cmd_ratio },
...
async fn compare_parallel_performance() {
    ...
    println!("Connections: {}, Commands per connection: {}", parallel_count, commands_per_conn);
    ...
    println!("  russh: {} connections, {} total commands in {:?}",
        russh_success, russh_success * commands_per_conn, russh_time);
    ...
    for _ in 0..parallel_count {
        let handle = tokio::spawn(async move {
            let config = ConnectionConfig::default();
            let conn = SshConnection::connect(HOST, 22, USER, None, &config).await.ok()?;
    ...
    println!("  ssh2: {} connections, {} total commands in {:?}",
        ssh2_success, ssh2_success * commands_per_conn, ssh2_time);

    // Comparison
    if russh_time.as_nanos() > 0 && ssh2_time.as_nanos() > 0 {
        let ratio = ssh2_time.as_nanos() as f64 / russh_time.as_nanos() as f64;
        println!("\n=== Parallel Comparison ===");
        println!("russh is {:.2}x {} than ssh2 for parallel workloads",
