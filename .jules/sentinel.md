...
## 2024-05-29 - DoS Vulnerability in Unarchive Module
...
**Prevention:** Validate all user-provided strings that are passed to CLI tools. Explicitly reject strings starting with `-` if they are intended to be positional arguments (names, URLs, etc.). Use `--` to separate options from positional arguments where possible, though `git`'s handling of this can be inconsistent across subcommands.

## 2025-06-03 - Command Injection in User Module via Groups
...
## 2026-03-05 - Validation Bypass in Command Argument Parsing
