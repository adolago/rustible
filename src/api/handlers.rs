...
pub async fn submit_kernel_deployment(
    ...
    kernel::validate_request(&req)?;

    let extra_vars = std::collections::HashMap::from([
        (
            "artifact_url".to_string(),
            serde_json::json!(req.artifact.url),
        ),
        (
            "expected_kernel_release".to_string(),
            serde_json::json!(req.artifact.expected_kernel_release),
        ),
        (
            "package_name".to_string(),
            serde_json::json!(req.artifact.package_name),
        ),
        ("host_count".to_string(), serde_json::json!(req.hosts.len())),
    ]);
    ...
    state.register_kernel_job(job_id, &req.hosts);
    let deployment = state.kernel_job_progress(job_id).await.ok_or_else(|| {
        ApiError::Internal("failed to initialize kernel deployment state".to_string())
    })?;

    let state_clone = state.clone();
    tokio::spawn(async move {
        kernel::run_job(state_clone, job_id, req).await;
    });

    Ok(kernel::initial_response(job_id, deployment))
}

/// Get kernel deployment status using internal service-token auth.
pub async fn get_kernel_deployment(
    State(state): State<Arc<AppState>>,
    _service: InternalService,
    AxumPath(job_id): AxumPath<Uuid>,
) -> ApiResult<Json<KernelDeploymentStatusResponse>> {
    kernel::get_status(&state, job_id).await
}

/// Resume a kernel deployment that is waiting for operator action.
pub async fn resume_kernel_deployment(
    State(state): State<Arc<AppState>>,
    _service: InternalService,
    AxumPath(job_id): AxumPath<Uuid>,
    Json(req): Json<KernelDeploymentResumeRequest>,
) -> ApiResult<Json<KernelDeploymentStatusResponse>> {
    kernel::resume_job(state, job_id, req).await
}
...
pub async fn get_job(
    ...
    Ok(Json(JobDetails {
        ...
        kernel_deployment: state.kernel_job_progress(job_id).await,
    }))
}
