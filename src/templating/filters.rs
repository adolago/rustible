...
fn filter_float(value: &Value) -> FilterResult<Value> {
    ...
    Ok(Value::Number(
        serde_json::Number::from_f64(n.unwrap_or(0.0)).unwrap_or_else(|| 0.into()),
    ))
}
...
fn filter_abs(value: &Value) -> FilterResult<Value> {
    ...
    Ok(Value::Number(
        serde_json::Number::from_f64(n.abs()).unwrap_or_else(|| 0.into()),
    ))
}
...
fn filter_round(value: &Value, args: &[Value]) -> FilterResult<Value> {
    ...
    Ok(Value::Number(
        serde_json::Number::from_f64(rounded).unwrap_or_else(|| 0.into()),
    ))
}
...
fn filter_product(value: &Value) -> FilterResult<Value> {
    ...
    Ok(Value::Number(
        serde_json::Number::from_f64(product).unwrap_or_else(|| 0.into()),
    ))
}
...
fn filter_mean(value: &Value) -> FilterResult<Value> {
    ...
    Ok(Value::Number(
        serde_json::Number::from_f64(mean).unwrap_or_else(|| 0.into()),
    ))
}
...
fn filter_median(value: &Value) -> FilterResult<Value> {
    ...
    Ok(Value::Number(
        serde_json::Number::from_f64(median).unwrap_or_else(|| 0.into()),
    ))
}
