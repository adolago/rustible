...
| Remote module coverage | Partial (verified list) | 18 modules are exercised against a live SSH target (see `Task::REMOTE_VERIFIED_MODULES`); a second list holds modules whose implementation is connection-only but which no environment here can exercise (systemd, firewalls, RPM distributions, mounts). Everything else is refused remotely rather than run against the control node. |
