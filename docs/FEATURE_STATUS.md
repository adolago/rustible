...
| Remote module coverage | Partial (verified list) | 22 modules are exercised against a live SSH target (see `Task::REMOTE_VERIFIED_MODULES`): apt, authorized_key, blockinfile, command, copy, cron, fetch, file, gather_facts, git, group, lineinfile, package, replace, script, setup, shell, slurp, stat, template, timezone, user. A second list holds 13 modules whose implementation is connection-only but which no environment here can exercise (systemd, firewalls, RPM distributions, mounts). Everything else is refused remotely rather than run against the control node. |
