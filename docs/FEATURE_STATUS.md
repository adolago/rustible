...
| Remote module coverage | Partial (verified list) | 27 entries on the verified list are exercised against a live SSH target, covering 26 registered modules — `setup` is the alias the executor resolves to `gather_facts` (see `Task::REMOTE_VERIFIED_MODULES`): apt, archive, authorized_key, blockinfile, command, copy, cron, fetch, file, gather_facts, git, group, lineinfile, package, ping, raw, replace, script, setup, shell, slurp, stat, template, timezone, unarchive, user, wait_for. A second list holds 20 modules whose implementation is connection-only but which no environment here can exercise (systemd, firewalls, RPM distributions, mounts, switches, PostgreSQL). A third names the 19 that run on the control node by design and are refused with a pointer to `delegate_to: localhost`. `tests/remote_transport_coverage_tests.rs` fails if a registered module belongs to none of them, so nothing is refused by omission. |
...
| Lock/checkpoint workflow | Partially verified | Lockfiles record the local files a playbook depends on, plus every role under `roles/` and every collection under `collections/ansible_collections`, each by a checksum over its file tree; `lock verify` detects an edit to any of them. A role that exists only as a Galaxy reference is not recorded, and `verify` still refuses to claim it checked an artifact it cannot read. |
